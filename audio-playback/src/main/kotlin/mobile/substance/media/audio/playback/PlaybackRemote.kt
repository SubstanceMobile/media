...
/**
 * This class is used to control all audio playback.
 */
object PlaybackRemote : ServiceConnection {
    ...
    fun <S : AudioService> withActivity(serviceClass: Class<S>, context: Context) {
        this.serviceClass = serviceClass
        this.context = context
        if (AudioPlaybackOptions.isCastEnabled) CastContext.getSharedInstance(context)

        if (SERVICE_BOUND_LISTENERS.isNotEmpty()) {
            val running = if (PlaybackRemote.context != null) AudioPlaybackUtil.isServiceRunning(PlaybackRemote.context!!, PlaybackRemote.serviceClass) else false
            if (running && isBound && service != null) {
                SERVICE_BOUND_LISTENERS.clear()
                return
            }
            if (!running) {
                Log.d("withActivity()", "Looks like the service is not running even though that has been requested. It will be started now")
                PlaybackRemote.context?.startService(Intent(PlaybackRemote.context, PlaybackRemote.serviceClass))
            }
            if (!isBound) PlaybackRemote.context?.bindService(Intent(PlaybackRemote.context, PlaybackRemote.serviceClass), this, Activity.BIND_ADJUST_WITH_ACTIVITY)
        }
    }
    ...
    fun registerCallback(callback: RemoteCallback) = getService { registerRemoteCallback(callback) }

    fun unregisterCallback(callback: RemoteCallback) = getService { unregisterRemoteCallback(callback) }
    ...
    interface PlayRequestCallback {

        fun onRequestPlayFromUri(uri: Uri?, extras: Bundle?)

        fun onRequestPlayFromMediaId(mediaId: String?, extras: Bundle?)

        fun onRequestPlayFromSearch(query: String, extras: Bundle)
    ...
    @JvmOverloads fun play(song: Song, replaceQueue: Boolean = true) {
        if (replaceQueue) {
            ...
            //Connect to the service and start playing audio. This is not recommended and may cause unpredictable behaviour
